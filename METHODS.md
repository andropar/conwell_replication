...
For computational tractability, most non-streamed analyses used a cached subset of layers from each model rather than storing every module output. Specifically, we ignored uninformative container or pass-through module types such as `Dropout`, `Identity`, `Flatten`, `Sequential`, and `ModuleList`, then sampled approximately ten layers from the second half of the remaining model depth, always retaining the final layer. Layer names were stored in the same `ModuleType-N` style used by DeepNSD.

The main odd/even split-half replication used a separate streamed extraction-to-fitting path that follows the released DeepNSD layer-search convention directly. In that analysis, each model's DeepNSD hook names were rediscovered and the full second half of the hook list was evaluated, without first filtering pass-through module types. Features were sparse-random-projection reduced in memory and immediately passed to the cRSA, voxel-encoding RSA, and SRPR evaluators; only score tables were written to disk. Thus, the split-half analysis uses a larger candidate layer set than the random, cluster, tau, OOD, and OOD-inclusive split-half analyses.
...
Because the candidate layer set differs across evaluation families, the plain split-half analysis should be interpreted as the full layer-search replication of Conwell et al.'s shared-image odd/even procedure. The other split families should be interpreted as computationally constrained generalization analyses over a precomputed approximately ten-layer second-half subset. Their absolute scores are therefore not a pure measure of split difficulty relative to the split-half run, although their within-family controlled model comparisons use a consistent layer-selection rule.
