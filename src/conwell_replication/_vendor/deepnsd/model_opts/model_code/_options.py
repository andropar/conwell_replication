...
def get_slip_model(model_name, verbose = True):
    slip_codebase_path = '{}/slip_codebase'.format(MODEL_CODE_DIR)
    sys.path.insert(0, slip_codebase_path)
    
    try:
        # Handle timm registry compatibility issue
        # Older timm versions had timm.models.registry, newer versions use register_model directly
        import timm
        if not hasattr(timm.models, 'registry'):
            # New timm API - create a compatibility shim
            # The decorator @timm.models.registry.register_model needs to work
            class RegistryShim:
                def register_model(self, fn):
                    # Register the model with timm's actual register_model function
                    timm.models.register_model(fn)
                    return fn
            timm.models.registry = RegistryShim()
        ...
        # PyTorch 2.6+ defaults to weights_only=True for security, but SLIP checkpoints
        # contain numpy arrays that require weights_only=False
        ...
        # Remove slip_codebase from path to avoid conflicts
        if slip_codebase_path in sys.path:
            sys.path.remove(slip_codebase_path)
...
# Import extended custom models
try:
    from .custom_extended_models import (
        define_openclip_options,
        define_cornet_options,
        define_robustness_options,
        define_dinov2_options,
        get_openclip_transforms,
        get_cornet_transforms,
        get_robustness_transforms,
        get_dinov2_transforms
    )
except ImportError:
    # If extended models module doesn't exist or has issues, define no-op functions
    ...
    def define_dinov2_options():
        return {}
    def get_openclip_transforms(*args, **kwargs):
        raise NotImplementedError("OpenCLIP models not available")
    def get_cornet_transforms(*args, **kwargs):
        raise NotImplementedError("CORnet models not available")
    def get_robustness_transforms(*args, **kwargs):
        raise NotImplementedError("Robustness models not available")
    def get_dinov2_transforms(*args, **kwargs):
        raise NotImplementedError("DINOv2 models not available")
...
custom_transform_options = {'slip': get_slip_transforms,
                            ...
                            'cornet': get_cornet_transforms,
                            'robustness': get_robustness_transforms,
                            'dinov2': get_dinov2_transforms}
...
def get_custom_transforms(model_query, input_type = 'PIL'):
    ...
    nonspecific_options = ['slip', 'seer', 'bit_expert', 'vicreg', 'vqgan', 'cornet', 'robustness', 'dinov2']
