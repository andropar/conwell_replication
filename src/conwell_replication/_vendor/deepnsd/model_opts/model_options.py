...
def get_dino_model(model_name):
    """Load DINO model, ensuring utils imports don't conflict with SLIP."""
    # Save current sys.path to restore later
    original_path = sys.path.copy()
    
    # Temporarily remove slip_codebase from path if it's there
    # This prevents torch.hub from accidentally importing SLIP's utils
    slip_codebase_path = os.path.join(os.path.dirname(__file__), 'model_code', 'slip_codebase')
    filtered_path = [p for p in sys.path if not p.endswith('slip_codebase')]
...
def define_dino_options():
    ...
    for index, row in dino_typology.iterrows():
        ...
        model_source = 'dino'
        model_string = '_'.join([model_name, train_type])
        model_call = "get_dino_model('{}')".format(model_name)
...
try:
    from model_opts.model_code._options import (
        get_custom_model_options, 
        get_custom_transforms_options,
        get_custom_transforms,
        custom_transform_options
    )
    _custom_models_available = True
except ImportError:
    _custom_models_available = False
    custom_transform_options = {}
