...
# Original DeepNSD imports — only needed for srp_extraction / pca_extraction
# which we do NOT use in this replication. Keep them lazy so that
# get_feature_map_srps (the only function we actually call) stays usable
# without the rest of DeepNSD.
try:
    from .feature_extraction import prep_model_for_extraction  # noqa: F401
    from .feature_extraction import get_all_feature_maps  # noqa: F401
    from .feature_extraction import check_model, get_prepped_model  # noqa: F401
except ImportError:
    prep_model_for_extraction = None
    get_all_feature_maps = None
    check_model = None
    get_prepped_model = None
...
def compute_jl_lemma(n_samples, eps=0.1):
    from sklearn.random_projection import johnson_lindenstrauss_min_dim
    return johnson_lindenstrauss_min_dim(n_samples, eps=eps)
...
def get_feature_map_srps(feature_maps, n_projections=None, upsampling=True, eps=0.1, seed=0,
    ...
    if n_projections is None:
        ...
        n_projections = compute_jl_lemma(n_samples, eps=eps)
...
def pca_extraction(model_string, model=None, inputs=None, feature_maps=None, 
                   ...
                   save_outputs=False, output_dir='temp_data/pca_arrays', 
                   delete_originals=False, delete_saved_outputs=True, verbose=False):
    ...
    if use_aux_pca:
        pca_args = {'feature_maps': aux_feature_maps, 
    ...
    if not use_aux_pca:
        pca_args = {'feature_maps': feature_maps, 
                    'n_components': n_components,
...
def rdm_extraction(model_string, model=None, model_inputs=None, feature_maps=None,
                   ...
                   save_outputs=False, output_dir='temp_data/rdm_arrays',
                   delete_originals=False, delete_saved_outputs=True, verbose=True):
